...
create index if not exists caminho_hospedagem_notificacoes_user_idx
  on public.caminho_hospedagem_notificacoes (user_id, created_at desc);
...
create policy caminho_hospedagem_aceites_own_all on public.caminho_hospedagem_aceites
for all to authenticated
using (user_id = auth.uid() or public.is_empresa_admin(tenant_id) or public.is_super_admin())
...
create policy caminho_hospedagem_avaliacoes_select_public on public.caminho_hospedagem_avaliacoes
for select to anon, authenticated using (publicada = true or cliente_id = auth.uid() or public.user_belongs_to_tenant(tenant_id) or public.is_super_admin());

drop policy if exists caminho_hospedagem_avaliacoes_insert_cliente on public.caminho_hospedagem_avaliacoes;
create policy caminho_hospedagem_avaliacoes_insert_cliente on public.caminho_hospedagem_avaliacoes
for insert to authenticated with check (cliente_id = auth.uid() and tenant_id = public.current_tenant_id());

drop policy if exists caminho_hospedagem_avaliacoes_update_admin on public.caminho_hospedagem_avaliacoes;
create policy caminho_hospedagem_avaliacoes_update_admin on public.caminho_hospedagem_avaliacoes
for update to authenticated using (public.is_empresa_admin(tenant_id) or public.is_super_admin()) with check (public.is_empresa_admin(tenant_id) or public.is_super_admin());

drop policy if exists caminho_hospedagem_favoritos_own_all on public.caminho_hospedagem_favoritos;
create policy caminho_hospedagem_favoritos_own_all on public.caminho_hospedagem_favoritos
for all to authenticated
using (user_id = auth.uid() or public.is_empresa_admin(tenant_id) or public.is_super_admin())
with check (user_id = auth.uid() and tenant_id = public.current_tenant_id());

drop policy if exists caminho_hospedagem_notificacoes_own_select on public.caminho_hospedagem_notificacoes;
create policy caminho_hospedagem_notificacoes_own_select on public.caminho_hospedagem_notificacoes
for select to authenticated using (user_id = auth.uid() or public.user_belongs_to_tenant(tenant_id) or public.is_super_admin());

drop policy if exists caminho_hospedagem_notificacoes_admin_insert on public.caminho_hospedagem_notificacoes;
create policy caminho_hospedagem_notificacoes_admin_insert on public.caminho_hospedagem_notificacoes
for insert to authenticated with check (public.is_empresa_admin(tenant_id) or public.is_super_admin() or user_id = auth.uid());

drop policy if exists caminho_hospedagem_notificacoes_own_update on public.caminho_hospedagem_notificacoes;
create policy caminho_hospedagem_notificacoes_own_update on public.caminho_hospedagem_notificacoes
for update to authenticated using (user_id = auth.uid() or public.is_empresa_admin(tenant_id) or public.is_super_admin()) with check (user_id = auth.uid() or public.is_empresa_admin(tenant_id) or public.is_super_admin());
